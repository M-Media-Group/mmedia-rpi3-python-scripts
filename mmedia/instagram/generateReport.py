...
def select_data_by_user(conn, profile_id):
	...
	"""
    cur = conn.cursor()
    cur.execute("SELECT * FROM accountsProgress WHERE profile_id=?",
                (str(profile_id)))

    rows = cur.fetchall()
    for row in rows:
        a, b, c, d, e, f = row
        dates_dict[str(datetime.strptime(e, "%Y-%m-%d %H:%M:%S").date())] = {
            'followers': b,
            'following': c,
            'posts': d
        }
...
def select_bot_data_by_user(conn, profile_id):
	...
	"""
    cur = conn.cursor()
    cur.execute("SELECT * FROM recordActivity WHERE profile_id=?",
                (str(profile_id)))

    rows = cur.fetchall()
    for row in rows:
        a, b, c, d, e, f, g = row
        dates_dict2[datetime.strptime(g, "%Y-%m-%d %H:%M:%S")] = {
            'likes': b,
            'comments': c,
            'follows': d,
            'unfollows': e,
    ...
    for date in dates_dict:
        ...
        for single_date in dates_dict2:
            ...
            if str(single_date.date()) > date or str(
                    single_date.date()) <= previous_date:
                continue
...
def calculateChangeInFollowers():
    ...
    for row in sorted(dates_dict):
        ...
        if previous == None:
            ...
            dates_dict[row]['change_in_followers'] = int(
                dates_dict[row]['followers']) - previous
        previous = int(dates_dict[row]['followers'])

    # Set other calculated vars
    best_date = max(dates_dict.keys(),
                    key=(lambda key: dates_dict[key]['change_in_followers']))
    worst_date = min(dates_dict.keys(),
                     key=(lambda key: dates_dict[key]['change_in_followers']))
    gain_followers = sum(dates_dict[row]['change_in_followers']
                         for row in dates_dict)
...
def calculateChangeInFollowing():
    ...
    for row in sorted(dates_dict):
        if previous == None:
            ...
            dates_dict[row]['change_in_following'] = int(
                dates_dict[row]['following']) - previous
        previous = int(dates_dict[row]['following'])
    gain_following = sum(dates_dict[row]['change_in_following']
                         for row in dates_dict)
...
def addToExcel():
    ...
    workbook.set_properties({
        ...
        'Instagram Stats',
        'subject':
        'Your Instagram stats from the Instagram Bot Management Service',
        'author':
        'M Media',
        'manager':
        'Mr. Michal Wargan',
        ...
        'Instagram data',
        'keywords':
        'Instagram, Data, Stats',
        'created':
        datetime.strptime(sorted(dates_dict)[-1], date_format),
        'comments':
    ...
    for row in sorted(dates_dict):
        worksheet.write_datetime(erow, col,
        ...
        worksheet.write(erow, col + 3,
                        int(dates_dict[row]['change_in_followers']))
        worksheet.write(erow, col + 4,
                        int(dates_dict[row]['change_in_following']))
        worksheet.write_formula(erow, col + 5,
                                '=C' + str(erow + 1) + '/B' + str(erow + 1))
        if erow > 1:
            worksheet.write_formula(erow, col + 6,
                                    '=F' + str(erow + 1) + '-F' + str(erow))
            worksheet.write_formula(erow, col + 8,
                                    '=H' + str(erow + 1) + '-H' + str(erow))
            worksheet.write_formula(erow, col + 10,
                                    '=J' + str(erow + 1) + '-J' + str(erow))
            worksheet.write_formula(erow, col + 12,
                                    '=L' + str(erow + 1) + '-L' + str(erow))
            worksheet.write_formula(erow, col + 14,
                                    '=N' + str(erow + 1) + '-N' + str(erow))
            worksheet.write_formula(erow, col + 16,
                                    '=P' + str(erow + 1) + '-P' + str(erow))
            worksheet.write_formula(erow, col + 18,
                                    '=R' + str(erow + 1) + '-R' + str(erow))
    ...
    worksheet.conditional_format('D2:D' + str(len(dates_dict) + 1),
                                 {'type': '3_color_scale'})
    worksheet.conditional_format('E2:E' + str(len(dates_dict) + 1),
                                 {'type': '3_color_scale'})
    worksheet.conditional_format('G2:G' + str(len(dates_dict) + 1), {
        'type': '3_color_scale',
        'max_color': '#F8696B',
        'min_color': '#63BE7B'
    })
    worksheet.conditional_format(
        'G2:G' + str(len(dates_dict) + 1), {
            'type':
            'icon_set',
            'icon_style':
            '3_arrows',
    ...
    chart.add_series({
        'name': '=Data!C1',
        'values': '=Data!$C$2:$C$' + str(len(dates_dict) + 1),
        'trendline': {
            'type': 'linear',
            'forward': 7,
            'display_r_squared': True,
    ...
    chart2.add_series({
        'name': '=Data!E1',
        'values': '=Data!$E$2:$E$' + str(len(dates_dict) + 1)
    })
    ...
    chart3.set_y_axis(
        {'name': 'People you follow for every follower you have'})
...
def sendEmail():
    ...
    body += '\n\n'
    body += "Followed by " + str(
        dates_dict[last_day]['followers']) + " people.\n"
    body += "Following " + str(dates_dict[last_day]['following']) + " people."

    body += '\n\n'
    body += "Best day was " + best_date + " with " + str(
        dates_dict[best_date]['change_in_followers']) + " new followers."

    body += '\n\n'
    body += "Net gain is " + str(
        gain_followers) + " new followers over " + str(
            run_days.days) + " days.\n\n"
    body += "Average recorded gain per log period is " + str(
        gain_followers / len(dates_dict)
    ) + " new followers. A log period is the time difference between two times I have checked your Instagram stats.\n\n"
    body += "Average calculated gain per day is " + str(
        gain_followers / run_days.days) + " new followers.\n\n"

    body += '\n\n'
    body += "Net new followed accounts is " + str(
        gain_following) + " over " + str(run_days.days) + " days.\n\n"
    body += "Average new followed accounts per log period is " + str(
        gain_following / len(dates_dict)) + ".\n\n"
    body += "Average new followed accounts per day is " + str(
        gain_following / run_days.days) + ".\n\n"

    body += '\n\n'
    body += "Net gain ratio of following to follower is " + str(
        gain_following / gain_followers) + ".\n\n"

    filename = "InstagramStats.xlsx"
    return memail.sendEmail(subject, body, config.settings['client']['email'],
                            filename)
...
def printData():
    ...
    print('\n')
    print("Best day is " + best_date + " with " +
          str(dates_dict[best_date][2]) + " new followers")
    print("Worst day is " + worst_date + " with " +
          str(dates_dict[worst_date][2]) + " new followers")

    print('\n')
    print("The net gain is " + str(gain_followers) + " new followers over " +
          str(run_days.days) + " days")
    print("The average recorded gain per log period is " +
          str(gain_followers / len(dates_dict)) + " new followers")
    print("The average calculated gain per day is " +
          str(gain_followers / run_days.days) + " new followers")

    print('\n')
    print("The net new followed accounts is " + str(gain_following) +
          " over " + str(run_days.days) + " days")
    print(
        "The recorded average new followed accounts per log period rate is " +
        str(gain_following / len(dates_dict)))
    print("The calculated average new followed accounts per day is " +
          str(gain_following / run_days.days))

    print('\n')
    print("The net gain ratio of following to follower is " +
          str(gain_following / gain_followers))

    print('\n')
    print("This clients expenditure is estimated at " + str(expenditure) +
          " EUR over " + str(diff_month(end_date, start_date)) + " months")
    print("This client spends on average " +
          str(expenditure / gain_followers) + " EUR per new follower")
